...
- `ÄRENDE_AVVISAT_PÅ_FORMALIA`, `ÄRENDE_BORDLAGT`
...
Händelser kan ha 0..n bilagor — filer, externa länkar eller textnoteringar som hör till den specifika händelsen. Typiska exempel: offerter, kvitton, läkarintyg, foton, underlag, externa myndighetsbeslut, kompletterande noteringar.

Tre bilage-typer stöds:

| Typ | Användning | Lagring |
|---|---|---|
| `FIL` | PDF, foton, dokument, kvitton, offerter | Bytes i db (BYTEA) |
| `URL` | Extern länk: leverantörs-offert, myndighetsbeslut online, artikel | URL-sträng + valfri snapshot |
| `TEXTNOTERING` | Kompletterande motivering, kontextkommentar utan eget dokument | Inline text |
...
bilaga {
  ...
  typ                  FIL / URL / TEXTNOTERING
  beskrivning          kort text — alla typer (t.ex. "Offert Asfalt AB 2026-03-12")
  innehålls_hash       SHA-256 — täcker innehållet enligt typen (se nedan)
  
  # Typ-specifika fält (populerade beroende på typ)
  ...
  -- URL
  url                  fullständig URL-sträng (max 2048 tecken)
  snapshot_taken_at    tidpunkt om snapshot togs (null annars)
  snapshot_bytes       BYTEA — sparad sidkopia vid registrering (null vid opt-out)
  
  -- TEXTNOTERING
  text                 fri text, max 5000 tecken
}
```

`innehålls_hash` beräknas typ-specifikt:

- **FIL:** `SHA-256(innehåll_bytes)`
- **URL:** `SHA-256(url || snapshot_bytes)` — täcker både länken och eventuell snapshot
- **TEXTNOTERING:** `SHA-256(text)` efter unicode NFC-normalisering
...
- **FIL:** 25 MB per bilaga, hård gräns; 100 MB total per händelse.
- **URL:** själva URL-strängen max 2048 tecken; snapshot ≤ 25 MB om sparad.
- **TEXTNOTERING:** max 5000 tecken (~en A4-sida); inga ytterligare gränser.

Gränserna är konfigurerbara via stadge-inställning om behov uppstår, men MVP-defaults är hårda.

### Tillåtna format

**FIL** — hårdkodad whitelist i binärens källkod, inte runtime-konfigurerbar och inte föreningsanpassbar.
...
**URL** — tillåtet: `http://` och `https://`. Blockerade scheman: `javascript:`, `data:`, `file:`, `ftp:` och liknande som öppnar säkerhetsrisker eller refererar lokalt innehåll.

**TEXTNOTERING** — ren UTF-8 text. Markdown-formatering tillåten i visning men lagras som källtext. Ingen HTML-rendering.

### URL-bilagor och länkröta

Externa URL:er kan upphöra fungera över tid (länkröta). Två strategier per URL-bilaga:

- **Standard:** systemet sparar bara URL-strängen + tidpunkten för registrering. Att länken inte längre fungerar är föreningens realitet — händelsen dokumenterar vad medlemmen *refererade till* vid tillfället.
- **Snapshot (opt-in per bilaga):** vid registrering hämtar systemet en kopia av sidan (HTML eller PDF) och sparar den som `snapshot_bytes`. Mer komplett bevaring men kostar lagring och har juridisk osäkerhet (upphovsrätt vid kopiering av tredje parts material).

Ingen automatisk snapshot-policy; användaren väljer per bilaga vid registrering.
...
Gallring av bilageinnehåll är **normalt systemhanterad** baserad på lagringstidskrav:
...
- Generellt: varje ärendetyp har en livscykelregel som styr när bilageinnehåll automatiskt ska gallras. Se [case-types.md](case-types.md) och [domain-model.md](domain-model.md).

**Gallring per bilage-typ:**

- **FIL:** `innehåll_bytes` nullas; metadata (`filnamn`, `innehållstyp`, `storlek`, `beskrivning`) + `innehålls_hash` bevaras.
- **URL:** `snapshot_bytes` nullas. Själva URL-strängen kan också gallras om den innehåller persondata (exempelvis URL till personprofil) — då ersätts den med `[gallrad]` och hashen bevaras separat.
- **TEXTNOTERING:** `text` nullas och ersätts med `[gallrad]`; metadata (`beskrivning`) + `innehålls_hash` bevaras. Granskaren kan se att en notering fanns och dess externa beskrivning, men inte innehållet.

I alla fall: granskaren ser att bilagan *fanns*, har dess `innehålls_hash`, och kan verifiera att kedjan inte är bruten.
...
3. Innehållet nullas enligt typ-regeln ovan; metadata bevaras.
